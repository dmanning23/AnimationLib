...
namespace AnimationTest
{
    ...
    public class Game1 : Microsoft.Xna.Framework.Game
    {
        ...
        protected override void Draw(GameTime gameTime)
        {
            ...
            Renderer.SpriteBatchBegin(BlendState.AlphaBlend, Renderer.Camera.TranslationMatrix * Resolution.TransformationMatrix(), SpriteSortMode.Immediate);
        ...
        /// <summary>
        /// Loads a sample Mai from KoF using the xml loader
        /// This character reuses some old animations from Apple (of Pajamorama fame) so many of the animations look goofy
        /// This character doesn't have any normal mapping
        /// </summary>
        ...
        public void LoadMai(ContentManager content)
        {
            ...
            //TODO: the content manager packs these from the json files
            ModelFile.SetRelFilename(@"Mai\MaiModel");
            animation.SetRelFilename(@"Mai\MaiAnimations");
            ...
            garmentFile.SetRelFilename(@"Mai\Clothes\Nekkid.xml");
        ...
        /// <summary>
        /// Loads a sample Mai from KoF using the json loader
        /// This character reuses some old animations from Apple (of Pajamorama fame) so many of the animations look goofy
        /// This character doesn't have any normal mapping
        /// </summary>
        /// <param name="content"></param>
        public void LoadMaiJson(ContentManager content)
        {
            ...
            Filename garmentFile = new Filename();
            garmentFile.SetRelFilename(@"Mai\Clothes\Nekkid.json");
        ...
        /// <summary>
        /// Loads a sample Dan from the wedding game using the Xml loader
        /// This character doesn't have any normal mapping
        /// </summary>
        /// <param name="content"></param>
        public void LoadDan(ContentManager content)
        {
            ModelFile = new Filename();
            Filename animation = new Filename();

            ModelFile.SetRelFilename(@"Dan\Dan model.xml");
            animation.SetRelFilename(@"Dan\Dan Animations.xml");

            Animations.ReadSkeletonXml(ModelFile, Renderer, content);
            Animations.ReadAnimationXml(animation, content);

            Filename garmentFile = new Filename();
            garmentFile.SetRelFilename(@"Dan\Clothes\Nekkid.xml");
            var garment = new Garment(garmentFile, Animations.Skeleton, Renderer, content, false);
            garment.AddToSkeleton();
            Garments.Add(garment);
        }

        private Garment LoadGarment(string garmentFile, ContentManager content = null)
        {
            if (!string.IsNullOrEmpty(garmentFile))
            {
                ...
                var garment = new Garment(filename, Animations.Skeleton, Renderer, content);
                garment.AddToSkeleton();
                Garments.Add(garment);
        ...
        private void LoadGrimoire(string modelFilename, string animationFilename, string garmentFilename = "", ContentManager content = null)
        {
            ModelFile = new Filename();
            Filename animation = new Filename();

            ModelFile.SetRelFilename(modelFilename);
            animation.SetRelFilename(animationFilename);

            Animations.ReadSkeletonXml(ModelFile, Renderer, content);
            Animations.ReadAnimationXml(animation, content);
            LoadGarment(garmentFilename, content);
        }

        /// <summary>
        /// Load the goblin from Grimoire Mi Amore
        /// This character uses normal mapping
        /// </summary>
        /// <param name="content"></param>
        public void LoadGrimoireGoblin(ContentManager content = null)
        {
            LoadGrimoire(@"Monsters\goblin\goblinBaseModel.xml",
                @"Monsters\goblin\goblinBaseAnimations.xml", string.Empty, content);

            var garmentFile = new Filename();
            garmentFile.SetFilenameRelativeToPath(ModelFile, @"Grunt\ax.xml");
            LoadGarment(garmentFile.File, content);
